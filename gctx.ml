...
type gctx = {
  ...
  y: int;
  color: color;
  thickness: thickness;   (** the current pen color *)
  }
...
let with_thickness (g: gctx) (c: thickness) : gctx =
  { g with thickness = c }
...
let set_graphics_state (gc: gctx) : unit =
  let c = gc.color in
  let d = gc.thickness in
  ((Graphics.set_color (Graphics.rgb c.r c.g c.b);
  (if d then Graphics.set_line_width 5
...
(**  Draw multiple points at the specified list of positions *)
let draw_points (g: gctx) (p1: position list) : unit =
  set_graphics_state g;
  let rec convert (t: position list) : position list =
   begin match t with
   | [] -> []
   | x :: z ->  (ocaml_coords g x) :: (convert z)
   end in
   let convertedlist = convert p1 in
   let rec pointy (t: position list) : unit =
   begin match t with
   |[] -> ()
   |(x,y)::z -> Graphics.plot x y; pointy z
   end in
   pointy (convertedlist)
